...
// ===== Career Matching Profiles =====
// แต่ละอาชีพจะอ้างอิงกับ "ตำแหน่งคำถาม" (0-63) ที่เกี่ยวข้องกับอาชีพนั้นมากที่สุด
// ตำแหน่งคำถามอ้างอิงจาก sections ด้านบน (ไล่ index 0-63 ตามลำดับ section/question)
const CAREER_PROFILES = [
  { name: "วิศวกรรมคอมพิวเตอร์", indices: [0, 1, 2, 3, 4, 21, 44, 56, 57, 59, 62] },
  { name: "Software Engineer / Developer", indices: [1, 2, 9, 18, 19, 38, 43, 57, 59, 63] },
  { name: "Data Scientist / AI Specialist", indices: [2, 4, 9, 19, 20, 40, 42, 59] },
  { name: "วิศวกรเครื่องกล / ไฟฟ้า", indices: [0, 4, 5, 8, 20, 22, 38, 43] },
  { name: "แพทย์ / สาธารณสุข", indices: [5, 12, 20, 27, 29, 55] },
  { name: "ครู / นักการศึกษา", indices: [12, 27, 41, 45, 55, 58] },
  { name: "ผู้ประกอบการ / ธุรกิจ", indices: [6, 13, 14, 25, 35, 50, 53] },
  { name: "นักออกแบบ / ครีเอทีฟ", indices: [7, 11, 23, 63] },
  { name: "นักวิจัย / นักวิทยาศาสตร์", indices: [5, 9, 10, 17, 20, 42] },
  { name: "นักจิตวิทยา / งานสังคม", indices: [12, 26, 27, 55] },
];

// คำนวณ % ความเหมาะสมของแต่ละอาชีพจากคำตอบ (1-5) แปลงเป็นสเกล 60-98%
// แล้วเรียงจากเหมาะสมที่สุดไปน้อยที่สุด
function calculateCareerMatch(answers) {
  return CAREER_PROFILES
    .map((c) => {
      const avg = c.indices.reduce((sum, i) => sum + answers[i], 0) / c.indices.length;
      const percent = Math.round(60 + ((avg - 1) / 4) * 38); // 1-5 -> 60-98
      return { name: c.name, percent: Math.min(98, Math.max(60, percent)) };
    })
...
function GoalPath({ goHome, userData, setRecommendedCareer, goPlanner }) {
  ...
  async function analyze() {
    ...
    // ===== 1) คำนวณ career / % match / top 3 ด้วยโค้ด (ไม่ใช้ AI) =====
    const careerMatches = calculateCareerMatch(answers);
    const topCareers = careerMatches.slice(0, 3);
    const career = topCareers[0].name;
    const percent = topCareers[0].percent;
    const totalScore = answers.reduce((s, v) => s + v, 0);

    // คะแนนเฉลี่ยรายหมวด (ใช้ให้ AI เข้าใจโปรไฟล์ผู้ใช้)
    const sectionSummary = sections.map((sec, si) => {
      const avg = (
        sec.questions.reduce((s, _q, qi) => s + answers[getIdx(si, qi)], 0) / sec.questions.length
      ).toFixed(1);
      return `- ${sec.title}: เฉลี่ย ${avg}/5`;
    }).join("\n");

    // ===== 2) ส่งผลที่คำนวณได้ ให้ AI อธิบายเหตุผล / จุดแข็ง / จุดพัฒนา / แผน 3 ปี =====
...
ตอบกลับเป็น JSON เท่านั้น ไม่มีข้อความอื่น ไม่มี markdown fences`;
    ...
    const userMessage = `ระบบคำนวณผลแบบทดสอบของ ${userData.name} อายุ ${userData.age} ปี ระดับ ${userData.education}${userData.job ? ` อาชีพ: ${userData.job}` : ""} ด้วยอัลกอริทึมแล้ว ได้ผลดังนี้:

อาชีพที่เหมาะที่สุด: ${career} (${percent}%)
Top 3 อาชีพที่เหมาะสม:
${topCareers.map((c, i) => `${i + 1}. ${c.name} (${c.percent}%)`).join("\n")}

คะแนนเฉลี่ยรายหมวด (จาก 8 ด้าน):
${sectionSummary}

คะแนนรวม: ${totalScore}/${totalQ * 5}

หน้าที่ของคุณ: อธิบายว่าทำไม ${userData.name} เหมาะกับ "${career}" โดยอ้างอิงคะแนนรายหมวดข้างต้น พร้อมระบุจุดแข็ง จุดที่ควรพัฒนา และวางแผนชีวิต 3 ปี

ตอบเป็น JSON รูปแบบนี้เท่านั้น (ห้ามเปลี่ยนชื่ออาชีพหรือเปอร์เซ็นต์ที่ให้มา):
{
  "summary": "อธิบายเหตุผลว่าทำไม ${userData.name} เหมาะกับ ${career} 2-3 ประโยค",
  ...
  "lifemap": [
    ...
    try {
      const aiData = await callClaude(systemPrompt, userMessage);

      // ===== 3) รวมผลที่คำนวณได้ (career/percent/topCareers/score) เข้ากับผลจาก AI =====
      const data = {
        career,
        percent,
        topCareers,
      ...
      if (typeof setRecommendedCareer === "function") setRecommendedCareer(career);
      localStorage.setItem("goalpathCareer", career);
      setResult(data);
    } catch (e) {
...
const s = {
  card: { background: "#fff", borderRadius: 24, padding: "clamp(20px, 6vw, 28px) clamp(14px, 5vw, 24px)", boxShadow: "0 8px 40px rgba(79,70,229,0.10)", border: "1px solid rgba(99,102,241,0.10)", width: "100%", maxWidth: 560, margin: "0 auto", boxSizing: "border-box" },
